(() => {
  ...
  // src/jff-canvas/ParticleView.js
  var require_ParticleView = __commonJS((exports) => {
    __export(exports, {
      default: () => ParticleView
    });
    const jff_canvas = __toModule(require_lib());
    class ParticleView extends jff_canvas.View {
      constructor(frame) {
        super(frame);
      ...
      fall(duration = 500) {
        this.animate(duration, this.setProgress, () => this.removeFromSuperview(), false, jff_canvas.Ease.EaseIn);
        this.startY = this.frame.y;
      }
      setProgress(progress, canvas) {
        this.frame.y = this.startY + 1e3 * progress;
      }
      ...
      animateFrame(newFrame, duration) {
        ...
        }, () => this.removeFromSuperview(), false, jff_canvas.Ease.Linear);
      }
    }
  });
  require_ParticleView();
})();
